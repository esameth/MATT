...
PDBData * LoadPDBFileReader(FileReader *FileReader, const char *suggestedName, int loadSeqres, int maxGaps, int caonly, int het) {
	PDBData *out = 0;
	...
	while ((i=readerGetLine(FileReader, &line, &lineLen)) > 0) {
		...
		if (loadSeqres && stristart("SEQRES", line)==0 && i>=21) {
			...
			for (h=0; h<(int)sizeof(HeaderOrder)/(int)sizeof(HeaderOrder[0]); h++) {
				if (stristart(HeaderOrder[h], line)==0) {
					...
					while (len) {
						...
						int h2;
						int pos = len-81;
						for (h2=0; h2<(int)sizeof(HeaderOrder)/(int)sizeof(HeaderOrder[0]); h2++) {
			...
			if (h<(int)sizeof(HeaderOrder)/(int)sizeof(HeaderOrder[0])) continue;
			for (h=0; h<(int)sizeof(CrystOrder)/(int)sizeof(CrystOrder[0]); h++) {
				if (stristart(CrystOrder[h], line)==0) {
					...
					while (len) {
						...
						int h2;
						int pos = len-81;
						for (h2=0; h2<(int)sizeof(CrystOrder)/(int)sizeof(CrystOrder[0]); h2++) {
	...
	for (i=0; i<out->numModels; i++) {
		if (cachedStrings[CACHED_DBREF]) {
		  string = cachedStrings[CACHED_DBREF];
			do {
				int start, end;
		...
		if (cachedStrings[CACHED_SEQADV]) {
		  string = cachedStrings[CACHED_SEQADV];
		  do {
				int res;
		...
		if (cachedStrings[CACHED_HELIX]) {
		  string = cachedStrings[CACHED_HELIX];
			do {
				int start, end;
				PDBChain * chain;
		...
		if (cachedStrings[CACHED_SHEET]) {
			...
			PDBChain * chain;
			BetaSheet *sheet = 0;
			string = cachedStrings[CACHED_SHEET];
			do {
		...
		if (cachedStrings[CACHED_SSBOND]) {
		  string = cachedStrings[CACHED_SSBOND];
			do {
				int res1, res2;
				PDBChain * chain;
				if (string[15] != string[29]) {
  			  string += 81;
  			  continue;
        }
	...
	if (cachedStrings[CACHED_CISPEP]) {
	  string = cachedStrings[CACHED_CISPEP];
		do {
			int res1, res2;
			int modelNum;
			if (!ReadInt(string+43, 3, &modelNum)) {
			  string += 81;
			  continue;
      }
			if (string[15] != string[29]) {
...
PDBData *LoadPDBFile(char *file, int seqres, int maxGaps, int caonly, int het) {
	...
	if (!reader) {
		if (DIR_DELIMITER != path[0] &&
			(DIR_DELIMITER != '\\' || path[1] != ':')) {
				...
				if (!reader && pdbPath) {
					char *search = strdup(pdbPath);
					char *prefix = strtok(search, ";");
					while (prefix && !reader) {
						int len = strlen(prefix) + 10 + strlen(path);
						char *path2 = (char*) malloc(2*len);
						char *path3 = path2+len;
						char subpath[5] = "";
						int nameLen = strcspn(name, ".");
						// For Astral names that have '.' as the chain name.
						if (nameLen == 5 && name[5] && name[6] && name[7] == '.') nameLen += 2;
						// Astral.
						if (nameLen == 7 && (name[0] == 'd' || name[0] == 'D')) {
							sprintf(subpath, "%c%c%c", DIR_DELIMITER, name[2], name[3]);
						}
						// PDB file name, with "pdb" prefix.
						else if (nameLen == 7 && strncasecmp(name, "pdb", 3)) {
						...
						if (path == name) {
							strcpy(path2, prefix);
							sprintf(path3, "%s%s", prefix, subpath);
						}
						else {
							sprintf(path2, "%s%c%s", prefix, DIR_DELIMITER, path);
							sprintf(path3, "%s%s", prefix, subpath);
	...
	if (reader) {
		out = LoadPDBFileReader(reader, name, seqres, maxGaps, caonly, het);
	}
	free(path);
	return out;
}
