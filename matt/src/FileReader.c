...
FileReader *readerOpenSub(char *path, int format) {
	FileReader *fileReader;
	FILE *file;
	if (!path || !(file = fopen(path, "rb"))) return 0;
	fileReader = (FileReader*) calloc(1, sizeof(FileReader));
	if (!fileReader || !(fileReader->path = strdup(path))) {
		free(fileReader);
		fclose(file);
		return 0;
	}
	...
	while (1) {
		...
		if ((!format || format == 1) &&
			(dfile = dopen(fileReader))) {
					FileReader *fileReader2 = (FileReader*) calloc(1, sizeof(FileReader));
					format = 0;
					if (!fileReader2) {
						dclose(dfile);
						return 0;
...
FileReader *readerOpen(char *name) {
	FileReader *fileReader = readerOpenSub(name, 0);
	if (!fileReader) {
		char *temp = (char*) malloc(sizeof(char)*(strlen(name)+10));
		sprintf(temp, "%s.Z", name);
		if (!(fileReader = readerOpenSub(temp, 1))) {
			sprintf(temp, "%s.gz", name);
			fileReader = readerOpenSub(temp, 2);
		}
		free(temp);
	}
...
const char* getFilePath(const FileReader* in) {
	return in->path;
}
