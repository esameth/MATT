...
int OutputAlignmentRmsd(MultipleAlignment *ma, const char *filename) {
  FILE *out = fopen(filename, "wb");
  int result = 0;
  int i, j;
  int coreCount = 0; int alnLength = ma->numResidues;
  if (!out) { return 0; }
    for (i=0; i<ma->numResidues;  i++) {
      // so this is the length of the alignment in core residues
      coreCount += IsCoreResidue(ma, i); // does it have coords?
    }
    fprintf(out, "# Formatt output on: ");
    OutputAlignmentOrder(ma, out, ma->order->root);
    fprintf(out, "\n");
    fprintf(out, "# core residues: %i\n", coreCount);
    fprintf(out, "# alignment length: %d\n# \n", alnLength);

    fprintf(out, "#\t");
    for(i=0; i < ma->numChains; i++) {
      fprintf(out, "\t%s", ma->chains[i]->pdb->idString);
    } fprintf(out, "\n");

    /////////////

    int *indices = (int *)malloc(sizeof(*indices) * ma->numChains);
    int chain;
    for (i=0; i<alnLength; i++) {
      int notcore = 0;
      double rmsdval = 99.99;
      for (chain=0; chain<ma->numChains; chain++) {
        if (ma->residues[chain].res[i].exists) {
          indices[chain] = ma->residues[chain].res[i].index;
        }
        else {
          indices[chain] = -1;
        }

        if (indices[chain] == -1) { notcore = 1; }
      }
      if (notcore == 0) {
	      rmsdval = CalcPositionAllPairsRMSD(ma, indices);
      }
	//	if (rmsdval > 3.00) rmsdval = 99.99;
  // if (rmsdval == 0.00) rmsdval = 99.99;
	    fprintf(out, "%2.2f\t%d",rmsdval,i);
    	for(j=0; j<ma->numChains; j++) {
    	  if (ma->residues[j].res[i].exists && ma->residues[j].res[i].residue != '~') {
    	    fprintf(out, "\t%c", ma->residues[j].res[i].residue);
    	  } else {
    	    fprintf(out, "\t-");
    	  }
    	}
    	fprintf(out, "\n");
	//} else if (notcore == 1) { notcore = 0; }
    }
